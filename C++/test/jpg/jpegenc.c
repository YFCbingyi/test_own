//
//  Copyright (C) 2016-2017  Dvtrend, Inc - All Rights Reserved
//
//  author: Wendong
//
...
struct JPEG_Enc
{
    // Tricky: In order to make it work, this MUST be first member of the structure
...
struct JPEG_Enc *new_jpegenc( int w, int h, int ch )
{
    ...
    jpeg_enc->jpegOutput = malloc(jpeg_enc->jpeg_w * jpeg_enc->jpeg_h * ch);  
    ...
    jpeg_enc->buffer = malloc(kBufferSize);
